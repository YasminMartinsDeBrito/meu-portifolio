export const Projects = () => {
  return (
    <div id="projects">
      <section className="projetos">
        <div className="kit">
          <h3>Jogo de Azar</h3>
          <a
            href="https://github.com/Kenzie-Academy-Brasil-Developers/jogo-de-azar-YasminMartinsDeBrito"
            target="_blank"
            rel="noopener noreferrer"
          >
            <img
              src="https://cf.shopee.com.br/file/fbdb8214e590f4f678e9adcb85a42340"
              alt=""
            />
            <p>GitHub: Repositorio</p>
          </a>
          <a
            href="https://kenzie-academy-brasil-developers.github.io/jogo-de-azar-YasminMartinsDeBrito"
            target="_blank"
            rel="noopener noreferrer"
          >
            <p>Vercel: Project</p>
          </a>
        </div>

        <div className="kit">
          <h3>Clone Instagram</h3>
          <a
            href="https://github.com/Kenzie-Academy-Brasil-Developers/rede-social-sprint-2-YasminMartinsDeBrito"
            target="_blank"
            rel="noopener noreferrer"
          >
            <img
              src="https://camo.githubusercontent."
              alt=""
            />
            <p>GitHub: Repositorio</p>
          </a>
          <a
            href="https://kenzie-academy-brasil-developers.github.io/rede-social-sprint-2-YasminMartinsDeBrito/"
            target="_blank"
            rel="noopener noreferrer"
          >
            <p>Vercel: Project</p>
          </a>
        </div>

        <div className="kit">
          <h3>ApexCharts</h3>
          <a
            href="https://github.com/YasminMartinsDeBrito/minehr"
            target="_blank"
            rel="noopener noreferrer"
          >
            <img
              src="https://i1.wp.com/gestaodesegurancaprivada.com.br/wp-content/uploads/definicao-de-grafico.jpg?fit=750%2C405&ssl=1"
              alt=""
            />
            <p>GitHub: Repositorio</p>
          </a>
          <a
            href="https://minehr.herokuapp.com/"
            target="_blank"
            rel="noopener noreferrer"
          >
            <p>Vercel: Project</p>
          </a>
        </div>

        <div className="kit">
          <h3>Rick and Morty</h3>
          <a
            href="https://github.com/YasminMartinsDeBrito/azs-web-rickandmorty"
            target="_blank"
            rel="noopener noreferrer"
          >
            <img
              src="https://br.web.img3.acsta.net/pictures/17/11/06/14/13/5164749.jpg?coixp=50&coiyp=47"
              alt=""
            />

            <p>GitHub: Repositorio</p>
          </a>
          <a
            href="https://azs-web-rickandmorty-1umqz9c0m-yasminmartinsdebrito.vercel.app/"
            target="_blank"
            rel="noopener noreferrer"
          >
            <p>Vercel: Project</p>
          </a>
        </div>
      </section>
    </div>
  );
};
