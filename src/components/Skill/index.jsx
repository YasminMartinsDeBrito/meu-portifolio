export const Skill = () => {
  return (
    <section id="skills">
      <ul className="lista-skill">
        <li className="skill">
          <div>
            <p>JavaScript</p>
            <img
              className="imagem"
              src="https://logospng.org/download/javascript/logo-javascript-1024.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>HTML5</p>
            <img
              className="imagem"
              src="https://cdn4.iconfinder.com/data/icons/logos-and-brands/512/167_Html5_logo_logos-512.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>CSS3</p>
            <img
              className="imagem"
              src="https://logospng.org/download/css-3/logo-css-3-2048.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>ReactJs</p>
            <img
              className="imagem"
              src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/2300px-React-icon.svg.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>API Rest</p>
            <img
              className="imagem"
              src="https://images.velog.io/images/hjkdw95/post/ca8c63d9-66c3-42da-8fb2-79f3a27c6876/api_rest.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>Styled-components</p>
            <img
              className="imagem"
              src="https://cdn.worldvectorlogo.com/logos/styled-components-1.svg"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>NodeJS</p>
            <img
              className="imagem"
              src="https://walde.co/wp-content/uploads/2016/09/nodejs_logo.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>Git</p>
            <img
              className="imagem"
              src="https://git-scm.com/images/logos/downloads/Git-Icon-1788C.png"
              alt=""
            />
          </div>
        </li>

        <li className="skill">
          <div>
            <p>Chakra UI</p>
            <img
              className="imagem"
              src="data:image/jpeg;base64,"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>PostgreSQL</p>
            <img
              className="imagem"
              src="https://brandlogos.net/wp-content/uploads/2021/11/postgresql-logo-512x512.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>Context-API</p>
            <img
              className="imagem"
              src="https://blog.azagatti.dev/static/209c7bf9d82339f86090771a3e4da9ef/07a9c/banner.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>Redux</p>
            <img
              className="imagem"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhmDtsYdvG33r593WvTsrbj7W9DLtUxIe5ug&usqp=CAU"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>TypeScript</p>
            <img
              className="imagem"
              src="https://iconape.com/wp-content/png_logo_vector/typescript.png"
              alt=""
            />
          </div>
        </li>
        <li className="skill">
          <div>
            <p>Graphql</p>
            <img
              className="imagem"
              src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/17/GraphQL_Logo.svg/2048px-GraphQL_Logo.svg.png"
              alt=""
            />
          </div>
        </li>
      </ul>
    </section>
  );
};
